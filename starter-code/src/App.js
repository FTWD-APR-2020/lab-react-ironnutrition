...
import "./App.css";
import foods from "./foods.json"; //Cannot add food here
import FoodBox from "./components/FoodBox";
import Search from "./Search";

class App extends Component {
  state = {
    allFoods: foods, //Can add food to this
    form: "",
    showForm: false,
  ...
  displayFoods = () => {
    return this.state.allFoods.map((eachFood, index) => {
      return (
        <FoodBox
          key={eachFood.name}
          name={eachFood.name}
  ...
  // pass down quantity via props
  // we display it
  // we create a listener
  // setState via handler of each individual food item
  //
  // when we click on a + we need to add something to state
  // use state to add li's

  quantityChangeHandler = (e, index) => {
    console.log(e.target.value, index);
    let foodItem = { ...this.state.allFoods[index], quantity: e.target.value };
    // let quantity = e.target.value;
    // foodItem.quantity = quantity;
    let copyOfFoods = [...this.state.allFoods];
    // splice(index, numberOfItemsToRemove, [itemsToAdd])
    copyOfFoods.splice(index, 1, foodItem);
    this.setState({
      allFoods: copyOfFoods,
    });
  };
